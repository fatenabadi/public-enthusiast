...
// Check if someone is trying to view another user's profile
if (isset($_GET['user_id']) && $_GET['user_id'] != $logged_in_user_id) {
    header("Location: publicenthusiastsprofile.php?user_id=" . $_GET['user_id']);
    exit();
}

// The profile being viewed (default to logged-in user's profile)
$profile_user_id = isset($_GET['user_id']) ? (int)$_GET['user_id'] : $logged_in_user_id;
...
// Initialize current step from URL or default based on whether user has data
if (isset($_GET['step']) && in_array($_GET['step'], [1, 2, 3])) {
    $currentStep = (int)$_GET['step'];
} else {
    // Default to step 3 if user has existing data, otherwise step 1
    $currentStep = (!empty($enthusiast_info) && !empty($art_preferences)) ? 3 : 1;
}
?>
...
<div class="profile-header" >
    ...
    <div class="container text-center">
        <div class="profile-image-container" id="profileContainer" >
            <img src="_Prompt for Enthusiast Cover Picture_A clean, modern cover image with the words “ENTHUSIAST”  in a .jpeg" class="profile-image" id="profileImg">
        </div>
...
function populateReview() {
    ...
    // Get selected artists - FIXED HERE
